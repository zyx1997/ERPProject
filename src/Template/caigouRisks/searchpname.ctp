        ...
        <section class="content">
			<div class="row">
				<form method="post" class="box box-primary" accept-charset="utf-8" enctype="multipart/form-data" action="/projectERP/caigouRisks/searchpname" style="width: 60%; margin: 0px auto;">
					...
					<div class="box-body" style="width:90%;margin: auto">
						<table class="table table-bordered table-hover" style="width:90%;margin: 0 auto;">
							...
							<tr>
								...
								<td><?=$result->shouji ?></td>
								<td><?=$result->dizhi ?></td>
								<?php if($result->pinggu == NULL ) { ?>
								<td><?= $this->Html->link('添加评估', ['action' => 'add', $result->p_id]) ?></td>
								<?php }else { ?>
								<td><?= $this->Html->link('修改评估', ['action' => 'modify', $result->id]) ?></td>
								<?php } ?>
							</tr>
							<?php endforeach; ?>
							</thead>

<?php
    ...
    class HetongListcheckYingxiaosController extends AppController
    {
		public  $paginate  =  [ 
			'limit'  =>  10, //每页显示条数
			'order'  =>  [ 
				'HetongLists.isshenhe_yingxiao'  =>  'asc', //顺序asc,倒序desc
        ...
        public function index()
        {
			...
			$this->loadComponent('Paginator');
			$hetongLists = $this->HetongLists->find('all', array('conditions' => array('HetongLists.istijiao'=>"1",'HetongLists.targetshenhe_yingxiao'=>"1")));
		...
		public function check($id)
        {
			...
			if ($this->request->is(['post', 'put'])) {
				$hetongList->shenheok_yingxiao = $this->request->getData('shenheok_yingxiao');
				$hetongList->shenhe_beizhu_yingxiao = $this->request->getData('shenhe_beizhu_yingxiao');
				$user = $this->request->getSession()->read('user'); 
				$hetongList->shenheren_yingxiao = $user->gonghao;
				$hetongList->isshenhe_yingxiao = "1"; 
				...
				$bumen_num = "1";
				if( $hetongList->shenheok_yingxiao == 1 ){ //当前为已审核通过
					//更新已审核通过的部门，注意正序排序
			...
			if ($this->request->is('post')) {
				...
				$hetongLists = $this->HetongLists->find(
					'all', array('conditions'=>array(
						...
						"HetongLists.targetshenhe_yingxiao = 1 ",

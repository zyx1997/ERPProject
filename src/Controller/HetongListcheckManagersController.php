<?php
    ...
    class HetongListcheckManagersController extends AppController
    {
		public  $paginate  =  [ 
			'limit'  =>  10, //每页显示条数
			'order'  =>  [ 
				'HetongLists.isshenhe_manager'  =>  'asc', //顺序asc,倒序desc
        ...
        public function index()
        {
			...
			$this->loadComponent('Paginator');
			$hetongLists = $this->HetongLists->find('all', array('conditions' => array('HetongLists.istijiao'=>"1",'HetongLists.keyishenhe_manager'=>"1")));
			...
			foreach($hetongLists as $hetongList){
				if($hetongList->shenheren_manager!=NULL){
		...
		public function check($id)
        {
			...
			if ($this->request->is(['post', 'put'])) {
				$hetongList->shenheok_manager = $this->request->getData('shenheok_manager');
				$hetongList->shenhe_beizhu_manager = $this->request->getData('shenhe_beizhu_manager');
				$user = $this->request->getSession()->read('user'); 
				$hetongList->shenheren_manager = $user->gonghao;
				$hetongList->isshenhe_manager = "1"; 
				...
				$bumen_num = "6";
				if( $hetongList->shenheok_manager == 1 ){ //当前为审核
					//更新已审核通过的部门，注意正序排序
			...
			if ($this->request->is('post')) {
				...
				$hetongLists = $this->HetongLists->find(
					'all', array('conditions'=>array(
						...
						"HetongLists.keyishenhe_manager = 1 ",

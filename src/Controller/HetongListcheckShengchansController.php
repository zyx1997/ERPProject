<?php
    ...
    class HetongListcheckShengchansController extends AppController
    {
		public  $paginate  =  [ 
			'limit'  =>  10, //每页显示条数
			'order'  =>  [ 
				'HetongLists.isshenhe_shengchan'  =>  'asc', //顺序asc,倒序desc
        ...
        public function index()
        {
			...
			$this->loadComponent('Paginator');
			$hetongLists = $this->HetongLists->find('all', array('conditions' => array('HetongLists.istijiao'=>"1",'HetongLists.targetshenhe_shengchan'=>"1")));
			...
			foreach($hetongLists as $hetongList){
				if($hetongList->shenheren_shengchan!=NULL){
		...
		public function check($id)
        {
			...
			if ($this->request->is(['post', 'put'])) {
				$hetongList->shenheok_shengchan = $this->request->getData('shenheok_shengchan');
				$hetongList->shenhe_beizhu_shengchan = $this->request->getData('shenhe_beizhu_shengchan');
				$user = $this->request->getSession()->read('user'); 
				$hetongList->shenheren_shengchan = $user->gonghao;
				$hetongList->isshenhe_shengchan = "1"; 
				...
				$bumen_num = "4";
				if( $hetongList->shenheok_shengchan == 1 ){ //当前为审核
					//更新已审核通过的部门，注意正序排序
			...
			if ($this->request->is('post')) {
				...
				$hetongLists = $this->HetongLists->find(
					'all', array('conditions'=>array(
						...
						"HetongLists.targetshenhe_shengchan = 1 ",

<?php
    ...
    class HetongListcheckCaigousController extends AppController
    {
		public  $paginate  =  [ 
			'limit'  =>  10, //每页显示条数
			'order'  =>  [ 
				'HetongLists.isshenhe_caigou'  =>  'asc', //顺序asc,倒序desc
        ...
        public function index()
        {
			...
			$this->loadComponent('Paginator');
			$hetongLists = $this->HetongLists->find('all', array('conditions' => array('HetongLists.istijiao'=>"1",'HetongLists.targetshenhe_caigou'=>"1")));
			...
			foreach($hetongLists as $hetongList){
				if($hetongList->shenheren_caigou!=NULL){
		...
		public function check($id)
        {
			...
			if ($this->request->is(['post', 'put'])) {
				$hetongList->shenheok_caigou = $this->request->getData('shenheok_caigou');
				$hetongList->shenhe_beizhu_caigou = $this->request->getData('shenhe_beizhu_caigou');
				//$this->HetongLists->patchEntity($hetongList, $this->request->getData());
				$user = $this->request->getSession()->read('user'); 
				$hetongList->shenheren_caigou = $user->gonghao;
				$hetongList->isshenhe_caigou = "1"; 
				...
				$bumen_num = "5";
				if( $hetongList->shenheok_caigou == 1 ){ //当前为审核
					//更新已审核通过的部门，注意正序排序
			...
			if ($this->request->is('post')) {
				...
				$hetongLists = $this->HetongLists->find(
					'all', array('conditions'=>array(
						...
						"HetongLists.targetshenhe_caigou = 1 ",

<?php
    ...
    class HetongListcheckZhijiansController extends AppController
    {
		public  $paginate  =  [ 
			'limit'  =>  10, //每页显示条数
			'order'  =>  [ 
				'HetongLists.isshenhe_zhijian'  =>  'asc', //顺序asc,倒序desc
        ...
        public function index()
        {
			...
			$this->loadComponent('Paginator');
			$hetongLists = $this->HetongLists->find('all', array('conditions' => array('HetongLists.istijiao'=>"1",'HetongLists.targetshenhe_zhijian'=>"1")));
			...
			foreach($hetongLists as $hetongList){
				if($hetongList->shenheren_zhijian!=NULL){
		...
		public function check($id)
        {
			...
			if ($this->request->is(['post', 'put'])) {
				$hetongList->shenheok_zhijian = $this->request->getData('shenheok_zhijian');
				$hetongList->shenhe_beizhu_zhijian = $this->request->getData('shenhe_beizhu_zhijian');
				$user = $this->request->getSession()->read('user'); 
				$hetongList->shenheren_zhijian = $user->gonghao;
				$hetongList->isshenhe_zhijian = "1"; 
				...
				$bumen_num = "3";
				if( $hetongList->shenheok_zhijian == 1 ){ //当前为审核
					//更新已审核通过的部门，注意正序排序
			...
			if ($this->request->is('post')) {
				...
				$hetongLists = $this->HetongLists->find(
					'all', array('conditions'=>array(
						...
						"HetongLists.targetshenhe_zhijian = 1 ",
